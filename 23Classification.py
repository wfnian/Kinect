...
def getData():
    f = open("F:\\kinect\\Kinect\\train_data.txt", 'r')
    data = f.readlines()
    y = []
    x = []
    for i in data:
...
class twentyclassification(nn.Module):
    def __init__(self, in_dim, n_hidden_1, n_hidden, n_hidden_2, out_dim):
        ...
        self.layer2 = nn.Sequential(
            nn.Linear(n_hidden_1, n_hidden), nn.ReLU(True))
        self.layer = nn.Sequential(
            nn.Linear(n_hidden, n_hidden_2), nn.ReLU(True))
